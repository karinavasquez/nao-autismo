...
checkRespuestaA = False
checkRespuestaB = False
checkRespuestaC = False
...
class ReactToTouch(ALModule):
    ...
    def onTouchedRight(self, subscriber, value):
        ...
        global checkIntroB
        global checkIntroC
        global checkSaludarA
        global checkSaludarB
        global checkRespuestaA
        global checkRespuestaB
        global checkRespuestaC
        global checkarmsUp
      ...
      # Condiciones para pasar los estados
        if value == 1.0:
            ...
            if counter == 1:
                ...
                checkSaludarB = False
                checkRespuestaA = True
                self.RespuestaAtencionA()
            
            elif counter == 7:
                checkRespuestaA = False
                checkRespuestaB = True
                self.RespuestaAtencionB()

            elif counter == 8:
                checkRespuestaB = False
                checkRespuestaC = True
                self.RespuestaAtencionC()
    ...
    def onTouchedLeft(self, subscriber, value):
        """ This will be called each time a touch
        is detected.
    ...
    def IntroB(self):            
        ...
        for y in range(0,1):    #Ilumina con todos los colores 3 veces
            if checkIntroB == True:
                for x in colors:
    ...
    def SaludarA(self):
        ...
        #1 Saludo verbal 
        if checkSaludarA == True:
           self.motionProxy.openHand('RHand')        
           self.talk("Hola", name)

    def SaludarB(self):       
        #2 Saludo Verbal + Mov. brazo derecho
        if checkSaludarB == True:
           self.talk("Hola", name)
           self.hiHand()
...
#  names = ["RElbowRoll","RShoulderRoll","RShoulderPitch","RElbowYaw","RWristYaw","RHand","HeadPitch", "HeadYaw","LElbowRoll","LShoulderRoll","LShoulderPitch","LElbowYaw","LWristYaw","LHand"]
    ...
    def recordVideoSound(self, filename):
        channels = (0,1,0,0)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ip", type=str, default="10.100.144.118",
