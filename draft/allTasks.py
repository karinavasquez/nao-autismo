...
#Codigo para ser utilizado con newMain que lleva a cabo los 5 modulos de autismo.

def Introduccion(entry):

    #1 Mueve la cabeza de un lado al otro, luego ve al nino
    #2 LEDS de ojos Parpadean 
    #3 Dice el nombre del nino
    ...
    anglesd = [0.0, 0.0]
    fractionMaxSpeed  = 0.05 #Varia de 0 a 1 donde 1 es mas rapido y 0 mas lento 
    ...
    for x in range(0,2):
        ...
        move(motionProxy, names, anglesd, fractionMaxSpeed) #Centro
        time.sleep(3.0)
    ...
    #2
    colors = ["white","red","green","blue", "yellow","magenta","cyan"] #Colores de los ojos del robot
    for y in range(0,3): #Ilumina con todos los colores 3 veces
        for x in colors:
            blinkColors(ledsProxy, x) #Utiliza la funcion "blinkColors" para el parpadeo de cada color
    time.sleep(2.0)
#MENOS TIEMPO (UN POCO)
    
    #3
    talk(ttsProxy, ledsProxy, name, spd, "") #Utiliza la funcion "talk" donde lo que dice el robot es el nombre
    time.sleep(2.0)
...
def Saludar(entry):
	...
	#Repite dos veces hola "adrian"

    #Saludo con el gesto
    hiHand(motionProxy, postureProxy, ttsProxy, ledsProxy, spd, name)
